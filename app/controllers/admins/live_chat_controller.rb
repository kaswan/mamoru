class Admins::LiveChatController < Admins::ApplicationController
    ...
    if params[:task] && !params[:room].blank?
      ...
      if params[:task] == 'getState'
        ...
        unless message.match(blankexp) 
          ...
          if current_admin_user.try(:image) 
            profile_image_path = current_admin_user.image.url(:icon)
          else  
          ...
          File.open(file, 'a') do |file|
            file.write("<div class='chat-box'><div class='chat-face'><img src=#{profile_image_path} alt='担当者名' width='80' height='80'></div><div class='chat-area'><div class='chat-editor'><div class='name'>#{current_admin_user.name}</div><div class='date'>#{(l Time.now, format: :day_hour_minute)} </div></div><div class='chat-hukidashi'>#{message} </div></div></div>\n")
          end
        end
        respond_to do |format|
          msg = { :state => '' }
